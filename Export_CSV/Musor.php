...
class Workly {
    ...
    public function insertData($arrivedAt, $leavedAt,$requiredWork,$workedOff) {
        try {
            ...
            $stmt->bindValue(':arrived_at', $arrivedAt->format('Y-m-d H:i:s'));
            $stmt->bindValue(':leaved_at', $leavedAt->format('Y-m-d H:i:s'));
            $stmt->bindValue(':required_work', $requiredWork);
            $stmt->bindValue(':worked_off', $workedOff);
            $stmt->execute();
    ...
    public function fetchAllRows() {
        try {
            $query = "SELECT * FROM daily LIMIT 5,5";
            $stmt = $this->pdo->query($query);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['arrived_at']) && isset($_POST['leaved_at'])) {
        ...
        $requiredWork = $arrivedAt->diff($leavedAt);
        $seconds = $requiredWork->format('%r%a days, %h hours, %i minutes, %s seconds');
        if ($seconds > 0) {
            $workedOff = 1;
        } else {
            $workedOff = 0;
